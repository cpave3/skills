...
Defines the JSON schemas used by the create-skill eval and benchmarking system.
...
- `current_best`: Best version identifier
- `iterations[].version`: Version ID (v0, v1, ...)
- `iterations[].parent`: Parent version
- `iterations[].expectation_pass_rate`: Pass rate from grading
- `iterations[].grading_result`: "baseline", "won", "lost", or "tie"
- `iterations[].is_current_best`: Whether this is current best
...
**Important:** The `expectations` array must use fields `text`, `passed`, and `evidence` — the viewer depends on these exact names.
...
**How to capture:** When a subagent completes, the notification includes `total_tokens` and `duration_ms`. Save immediately — not persisted anywhere else.
...
**Important:** The viewer reads field names exactly. Using `config` instead of `configuration`, or `pass_rate` at the top level instead of nested under `result`, causes the viewer to show empty values.

**Fields:**
- `metadata`: Benchmark run info (skill_name, timestamp, evals_run, runs_per_configuration)
- `runs[]`: Individual results (eval_id, eval_name, configuration must be `"with_skill"` or `"without_skill"`, run_number, result with pass_rate/passed/total/time_seconds/tokens/errors)
- `run_summary`: Statistical aggregates with mean/stddev/min/max per configuration, plus delta
